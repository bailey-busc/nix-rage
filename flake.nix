    ...
    inputs.flake-parts.lib.mkFlake { inherit inputs; } {
      ...
      perSystem =
        ...
        let
          ...
          nix_pkgs = lib.filterAttrs (
            nix_version: nix_pkg:
            (
              lib.elem nix_version [
                "git"
                "latest"
                "minimum"
                "stable"
        ...
        in
        {
            ...
            in
            {
              "testPlugin-nix-${nix_version}" = pkgs.testers.runNixOSTest {
                ...
                nodes.machine1 = {
                  nix.package = nix_pkg;
                  nix.extraOptions = ''
                    plugin-files = ${self'.packages.nix-rage.override ({ nix = nix_pkg; })}/lib/libnix_rage.so
          ...
          packages = {
            nix-rage = pkgs.callPackage ./package.nix {
              nix = pkgs.nixStable;
              inherit craneLib;
            };
